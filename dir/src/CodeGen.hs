...
codeGen :: FilePath -> Program -> Err String
...
compileDef (DFun rt f@(Id name) args ss) = do
    let argTypes = concatMap (\ (ADecl rt _) -> typeToStr rt) args
    emitBlank
    emitText $ ".method public static " ++ name ++ "(" ++ argTypes ++ ")" ++ typeToStr rt
    emitText $ "  .limit locals 1000" 
    emitText $ "  .limit stack 1000"  
    
    newBlock
    extendVars $ Prelude.map (\(ADecl _ id) -> id) args
    compileStms ss
    exitBlock
